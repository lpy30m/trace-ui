...
- 增强 **Dispatcher pointer-memory capture**：multi-dispatcher 脚本允许用户显式选择 X0-X7 和每个指针的最大读取字节数；捕获结果复用 `byteArray`/`readError` 协议，可直接生成后续 angr memory seed。默认不读取任何指针，所有读取有界且由用户手动运行脚本。
