...
where
    F: FnMut(u32, u32) -> bool,
{
    traverse_forward_index_with_extra(
        index,
        start_indices,
        data_only,
        max_nodes,
...
pub fn traverse_forward_index_with_extra<F>(
    index: &ForwardDependencyIndex,
    start_indices: &[u32],
    data_only: bool,
    max_nodes: usize,
    extra_consumers: &std::collections::HashMap<u32, Vec<u32>>,
    mut checkpoint: F,
) -> Option<ForwardTraversal>
where
    F: FnMut(u32, u32) -> bool,
{
    ...
    while let Some(source) = queue.pop_front() {
        ...
        if let Some(consumers) = extra_consumers.get(&source) {
            for &consumer in consumers {
                let consumer = consumer & LINE_MASK;
                if consumer as usize >= line_count {
                    continue;
                }
                traversed_edges = traversed_edges.saturating_add(1);
                if affected[consumer as usize] {
...
mod tests {
    ...
    #[test]
    fn follows_extra_call_boundary_consumers() {
        let trace = [
            r#"[00:00:00 001][lib.so 0x100] [d2800548] 0x40000100: "mov x8, #42" => x8=0x2a"#,
            r#"[00:00:00 001][lib.so 0x104] [d503201f] 0x40000104: "nop""#,
            r#"[00:00:00 001][lib.so 0x108] [d63f0020] 0x40000108: "blr x1" x1=0x50000000"#,
        ]
        .join("\n");
        ...
        let index = build_forward_index(&view, |_, _| true).unwrap();
        let extra = std::collections::HashMap::from([(0, vec![2])]);
        let result =
            traverse_forward_index_with_extra(&index, &[0], true, 100, &extra, |_, _| true)
                .unwrap();
        assert!(result.affected[0]);
        assert!(result.affected[2]);
        assert!(!result.terminal_seqs.contains(&0));
        assert!(result.terminal_seqs.contains(&2));
    }
    ...
    fn follows_aapcs_argument_into_branch_link_call() {
        ...
        assert!(
            !result.affected[2],
            "unrelated register definition must be excluded"
        );
