...
pub fn software_shape_class(mnemonic: &str) -> Option<&'static str> {
    match mnemonic {
        ...
        "rev" | "rev16" | "rev32" | "rbit" | "ext" | "tbl" | "tbx" | "zip1" | "zip2" | "uzp1"
        | "uzp2" | "trn1" | "trn2" => Some("permutation"),
        "lsl" | "lsr" | "asr" | "ror" | "shl" | "sshr" | "ushr" | "sli" | "sri" => Some("shift"),
        _ => None,
    }
}
...
pub struct CryptoFunctionReport {
    ...
    pub zero_result_explanation: Option<String>,
}
...
pub fn aggregate_signals(
    ...
    for (fid, acc) in by_func {
        let boolean = acc.shape_counts.get("boolean").copied().unwrap_or(0);
        ...
        let mut algorithms = acc.algorithms;
        let mut implementation_hints = Vec::new();
...
pub fn score_candidate(raw: &RawFunctionSignals) -> EvidenceAssessment {
    ...
    let signals = vec![
        ...
        EvidenceScoreSignal::new(
            "dense_boolean_network",
            "Function executes a dense boolean network without dedicated crypto instructions.",
            30,
            bitsliced_candidate,
            bitsliced_candidate.then(|| format!("boolean_ops={boolean_ops}")),
        ),
        EvidenceScoreSignal::new(
            "bit_permutation_mixing",
            "Boolean network is corroborated by shift/permutation operations.",
            15,
            bitsliced_candidate,
            bitsliced_candidate.then(|| format!("mixing_ops={mixing_ops}")),
        ),
        EvidenceScoreSignal::new(
...
mod tests {
    ...
    fn insn_family_mapping() {
        ...
        assert_eq!(software_shape_class("eor"), Some("boolean"));
        assert_eq!(software_shape_class("tbl"), Some("permutation"));
        assert_eq!(software_shape_class("ror"), Some("shift"));
        assert_eq!(software_shape_class("add"), None);
    }
    ...
    #[test]
    fn dedicated_instructions_are_related_not_verified() {
        let t = sample_tree();
        ...
        let a = score_candidate(raw);
        assert_eq!(a.grade, "related");
        assert_eq!(a.confidence, "medium");
        assert!(!a.verification_gate_met);
    }
    ...
    #[test]
    fn dense_boolean_network_is_related_bitsliced_candidate_not_verified() {
        let t = sample_tree();
        let mut shapes = Vec::new();
        shapes.extend((0..64).map(|_| SoftwareShapeHit {
            seq: 25,
            class: "boolean",
        }));
        shapes.extend((0..16).map(|_| SoftwareShapeHit {
            seq: 25,
            class: "permutation",
        }));
        let raws = aggregate_signals(&[], &[], &shapes, &t);
        let raw = raws.iter().find(|raw| raw.func_id == 2).unwrap();
        assert_eq!(raw.software_shape_total, 80);
        assert_eq!(raw.implementation_hints, vec!["BitslicedSoftware"]);
        let assessment = score_candidate(raw);
        assert_eq!(assessment.grade, "related");
        assert!(!assessment.verification_gate_met);
    }

    #[test]
    fn ordinary_bitwise_function_below_threshold_is_not_bitsliced() {
        let t = sample_tree();
        let mut shapes = Vec::new();
        shapes.extend((0..63).map(|_| SoftwareShapeHit {
            seq: 25,
            class: "boolean",
        }));
        shapes.extend((0..15).map(|_| SoftwareShapeHit {
            seq: 25,
            class: "shift",
        }));
        let raws = aggregate_signals(&[], &[], &shapes, &t);
        assert!(raws.iter().all(|raw| !raw
            .implementation_hints
            .iter()
            .any(|hint| hint == "BitslicedSoftware")));
    }

    #[test]
    fn sm4_instruction_shape_remains_related_until_semantic_recomputation() {
        let t = sample_tree();
        let insns: Vec<CryptoInsnHit> = (0..6)
            .map(|i| CryptoInsnHit {
                seq: 22 + i,
                family: CryptoFamily::Sm4,
            })
            .collect();
        let raw = aggregate_signals(&[], &insns, &[], &t)
            .into_iter()
            .find(|raw| raw.func_id == 2)
            .unwrap();
        assert_eq!(raw.algorithms, vec!["SM4"]);
        let assessment = score_candidate(&raw);
        assert_eq!(assessment.grade, "related");
        assert!(!assessment.verification_gate_met);
    }
    ...
    #[test]
    fn des_constant_shape_remains_related_until_semantic_recomputation() {
        let t = sample_tree();
        let magic: Vec<CryptoMagicHit> = ["0xFEE1A2B3", "0xD7BEF080", "0x3A322A22"]
            .iter()
            .enumerate()
            .map(|(i, value)| CryptoMagicHit {
                seq: 22 + i as u32,
                algorithm: "DES1".into(),
                magic_hex: (*value).into(),
            })
            .collect();
        let raw = aggregate_signals(&magic, &[], &[], &t)
            .into_iter()
            .find(|raw| raw.func_id == 2)
            .unwrap();
        assert_eq!(raw.base_family_count, 1);
