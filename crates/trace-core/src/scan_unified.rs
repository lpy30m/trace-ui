...
use crate::query::strings::StringBuilder;
use crate::scanner::{
    self as scanner, mem_access_width, push_unique, PairSplitDeps, RegLastDef, ScanState,
...
pub struct ScanResult {
    ...
    pub format: trace_parser::types::TraceFormat,
    pub call_annotations: std::collections::HashMap<u32, trace_parser::gumtrace::CallAnnotation>,
    pub consumed_seqs: Vec<u32>,
}

/// 将字节序列转为字符串：有效 UTF-8 部分正常解码，无效字节显示为 `\xNN` 形式。
pub fn bytes_to_hex_escaped(bytes: &[u8]) -> String {
    ...
    while pos < len {
        ...
        if format == types::TraceFormat::Gumtrace && gumtrace_parser::is_special_line(raw_line) {
            ...
            if let Some(special) = gumtrace_parser::parse_special_line(raw_line) {
                ...
                match special {
                    gumtrace_parser::SpecialLine::CallFunc {
                        name,
                        is_jni,
                        raw_args,
                    } => {
                    ...
                    gumtrace_parser::SpecialLine::HexDump => {
                        ...
                        if let Some((_, ref mut ann)) = current_annotation {
                            ann.observation_seq.get_or_insert(i);
                            ann.raw_lines.push(raw_line.to_string());
                        }
                    }
                }
