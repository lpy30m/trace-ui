...
#[derive(Clone, Copy)]
enum ModeledMemoryValue {
    Copy { source: u64 },
    Fill { value: u8 },
}

#[derive(Clone, Copy)]
struct ModeledMemoryWrite {
    seq: u32,
    destination: u64,
    length: u64,
    value: ModeledMemoryValue,
}

fn annotation_hex_arg(
...
fn modeled_memory_writes(
    annotations: &HashMap<u32, trace_parser::gumtrace::CallAnnotation>,
    limit: u32,
) -> Vec<ModeledMemoryWrite> {
    let mut writes = Vec::new();
    for (&entry_seq, annotation) in annotations.iter() {
        let seq = annotation.completion_seq.unwrap_or(entry_seq);
        if seq > limit {
            continue;
        }
        let name = annotation
        ...
        match name.as_str() {
            "memcpy" | "memmove" => {
                if let (Some(destination), Some(source), Some(length)) = (
                    annotation_hex_arg(annotation, "x0"),
                    annotation_hex_arg(annotation, "x1"),
                    annotation_hex_arg(annotation, "x2"),
                ) {
                    writes.push(ModeledMemoryWrite {
                        seq,
                        destination,
                        length,
                        value: ModeledMemoryValue::Copy { source },
                    });
                }
            }
            "strcpy" => {
                if let (Some(destination), Some(source)) = (
                    ...
                    writes.push(ModeledMemoryWrite {
                        seq,
                        destination,
                        length: decoded_argument_length(annotation, "1").unwrap_or(255) + 1,
                        value: ModeledMemoryValue::Copy { source },
                    });
                }
            }
            "strcat" => {
                if let (Some(destination), Some(source)) = (
                    ...
                    let old_length = decoded_argument_length(annotation, "0").unwrap_or(0);
                    let added_length = decoded_argument_length(annotation, "1").unwrap_or(255);
                    writes.push(ModeledMemoryWrite {
                        seq,
                        destination: destination.saturating_add(old_length),
            ...
            "memset" => {
                if let (Some(destination), Some(value), Some(length)) = (
                    annotation_hex_arg(annotation, "x0"),
                    annotation_hex_arg(annotation, "x1"),
                    annotation_hex_arg(annotation, "x2"),
                ) {
                    writes.push(ModeledMemoryWrite {
                        ...
                        destination,
                        length,
                        value: ModeledMemoryValue::Fill { value: value as u8 },
                    });
                }
            }
            _ => {}
...
#[cfg(test)]
mod call_hexdump_integration_tests {
    use crate::{BuildOptions, SliceOptions, TraceEngine};

    #[test]
    #[ignore = "requires TRACE_MD5_WRAPPER_SAMPLE pointing to the private trace"]
    fn real_wrapper_trace_exposes_final_buffer_with_call_provenance() {
        let path = std::env::var("TRACE_MD5_WRAPPER_SAMPLE")
            .expect("TRACE_MD5_WRAPPER_SAMPLE is required");
        ...
        engine
            ...
            .unwrap();
        let snapshot = engine
            .get_memory_at(&session.session_id, 0x7fd84c7ad0, 1_090_900, 0x24)
            .unwrap();
        assert_eq!(
            &snapshot.bytes[..0x24],
            b"774530081208515772211060627361376815"
        );
        assert!(snapshot.known[..0x24].iter().all(|known| *known));
        assert!(snapshot.provenance[..0x24]
            .iter()
            .all(|p| p.source == "call_hexdump"));
        let slice = engine
            .run_slice(
                &session.session_id,
                &["mem:0x7fd84c7ad0:36@1090869".to_string()],
                SliceOptions {
                    start_seq: None,
                    end_seq: None,
                    data_only: true,
            ...
            .unwrap();
        assert!(
            slice.marked_count > 10,
            "call-effect slice should cross the memmove boundary"
        );
        let tainted = engine.get_tainted_seqs(&session.session_id).unwrap();
        assert!(
            tainted
                .iter()
                .any(|seq| (1_090_710..=1_090_744).contains(seq)),
            "slice should reach strcat inputs"
        );
        assert!(
            tainted
                .iter()
                .any(|seq| (1_090_210..=1_090_711).contains(seq)),
            "slice should reach sprintf producers"
        );
    }

    #[test]
    #[ignore = "requires TRACE_MD5_CORE_SAMPLE pointing to the private trace"]
    fn real_core_trace_has_the_same_format_join_copy_call_shape() {
        let path =
            std::env::var("TRACE_MD5_CORE_SAMPLE").expect("TRACE_MD5_CORE_SAMPLE is required");
        let engine = TraceEngine::new();
        let session = engine.create_session(&path).unwrap();
        engine
            .build_index(
        ...
        let snapshot = engine
            .get_memory_at(&session.session_id, 0x7fd84c7ad0, 1_040_310, 0x26)
            .unwrap();
        assert_eq!(
            &snapshot.bytes[..0x26],
            b"17983072012217347211048457368218090083"
        );
        assert!(snapshot.known[..0x26].iter().all(|known| *known));

        let call_seqs = [
            1_039_630, 1_039_828, 1_039_974, 1_040_120, 1_040_131, 1_040_142, 1_040_153, 1_040_281,
        ];
        let shape: Vec<_> = engine
            .get_lines(&session.session_id, &call_seqs)
            .unwrap()
            .into_iter()
            .filter_map(|line| line.call_info.map(|call| call.func_name))
            .collect();
        assert_eq!(
            shape,
            ["sprintf", "sprintf", "sprintf", "sprintf", "strcat", "strcat", "strcat", "memmove"]
        );

        let slice = engine
            .run_slice(
                &session.session_id,
                &["mem:0x7fd84c7ad0:38@1040282".to_string()],
                SliceOptions {
                    start_seq: None,
                    end_seq: None,
                    data_only: true,
            ...
            .unwrap();
        assert!(slice.marked_count > 10);
        let tainted = engine.get_tainted_seqs(&session.session_id).unwrap();
        assert!(tainted
            .iter()
            .any(|seq| (1_040_125..=1_040_160).contains(seq)));
        assert!(tainted
            .iter()
            .any(|seq| (1_039_620..=1_040_121).contains(seq)));
    }
}

#[cfg(test)]
mod call_model_tests {
    use super::{modeled_memory_writes, ModeledMemoryValue};
    use std::collections::HashMap;
    use trace_parser::gumtrace::CallAnnotation;

    #[test]
    fn derives_copy_fill_and_append_destination_writes() {
        let annotations = HashMap::from([
            (
                10,
                CallAnnotation {
                    func_name: "memmove".into(),
                    is_jni: false,
                    args: vec![
                        ("x0".into(), "0x2000".into()),
                        ("x1".into(), "0x1000".into()),
        ...
        let writes = modeled_memory_writes(&annotations, 20);
        assert_eq!(writes.len(), 2);
        assert_eq!(writes[0].destination, 0x2000);
        assert_eq!(writes[0].length, 0x20);
        assert!(matches!(
            writes[0].value,
            ModeledMemoryValue::Copy { source: 0x1000 }
        ));
        assert!(matches!(
            writes[1].value,
            ModeledMemoryValue::Fill { value: 0x41 }
        ));
    }
}
...
impl super::TraceEngine {
    ...
    pub fn get_memory_at(
        ...
        let mut provenance = Vec::with_capacity(len);
        let hexdumps: Vec<_> = state
            .call_annotations
            .iter()
            .filter(|(call_seq, _)| **call_seq <= seq)
            .flat_map(|(&call_seq, annotation)| {
                annotation
                    .hexdump_observations()
                    .into_iter()
                    .map(move |observation| {
                        (observation.observation_seq.unwrap_or(call_seq), observation)
                    })
            })
            .filter(|(observation_seq, _)| *observation_seq <= seq)
            .collect();
        let modeled_writes = modeled_memory_writes(&state.call_annotations, seq);
        ...
        for offset in 0..len {
            ...
            let mut hexdump_byte: Option<(u32, u8)> = None;
            for (call_seq, observation) in &hexdumps {
                let end = observation
                    .base_addr
                    .saturating_add(observation.bytes.len() as u64);
                if byte_addr >= observation.base_addr
                    && byte_addr < end
                    && hexdump_byte
                        .as_ref()
                        .is_none_or(|(seen, _)| *call_seq >= *seen)
                {
                    hexdump_byte = Some((
                        *call_seq,
                        observation.bytes[(byte_addr - observation.base_addr) as usize],
                    ));
                }
            }
            let hexdump_won = hexdump_byte.is_some_and(|(call_seq, _)| {
                best_seq.is_none_or(|instruction_seq| call_seq >= instruction_seq)
            });
            if let Some((call_seq, value)) = hexdump_byte.filter(|(call_seq, _)| {
                best_seq.is_none_or(|instruction_seq| *call_seq >= instruction_seq)
            }) {
                bytes[offset] = value;
                known[offset] = true;
                best_seq = Some(call_seq);
            }
            let mut model_won = false;
            if let Some(write) = modeled_writes.iter().rev().find(|write| {
                byte_addr >= write.destination
                    && byte_addr < write.destination.saturating_add(write.length)
                    && best_seq.is_none_or(|observed_seq| write.seq > observed_seq)
            }) {
                let modeled_value = match write.value {
                    ModeledMemoryValue::Fill { value } => Some(value),
                    ModeledMemoryValue::Copy { source } => {
                        let source_addr = source.saturating_add(byte_addr - write.destination);
                        let mut source_value: Option<(u32, u8)> = None;
                        for check_offset in 0u64..=7 {
                            if source_addr < check_offset {
                                continue;
                            }
                            if let Some(records) = mem_view.query(source_addr - check_offset) {
                                let pos = records.partition_point(|record| record.seq <= write.seq);
                                if pos > 0 {
                                    let record = &records[pos - 1];
                                    if check_offset < record.size as u64
                                        && source_value.is_none_or(|(seen, _)| record.seq > seen)
                                    {
                                        source_value = Some((
                                            record.seq,
                                            ((record.data >> (check_offset * 8)) & 0xff) as u8,
                                        ));
                                    }
                                }
                            }
                        ...
                        for (call_seq, observation) in &hexdumps {
                            let end = observation
                                .base_addr
                                .saturating_add(observation.bytes.len() as u64);
                            if *call_seq <= write.seq
                                && source_addr >= observation.base_addr
                                && source_addr < end
                                && source_value.is_none_or(|(seen, _)| *call_seq >= seen)
                            {
                                source_value = Some((
                                    *call_seq,
                                    observation.bytes
                                        [(source_addr - observation.base_addr) as usize],
                                ));
                            }
                        }
                        source_value.map(|(_, value)| value)
                    }
                };
                if let Some(value) = modeled_value {
                    bytes[offset] = value;
                    known[offset] = true;
                    best_seq = Some(write.seq);
                    model_won = true;
                }
            }
            provenance.push(MemoryByteProvenance {
                offset: offset as u32,
                address: format!("0x{:x}", byte_addr),
                source: if model_won {
                    "call_model".to_string()
                } else if hexdump_won {
                    "call_hexdump".to_string()
                } else if best_seq.is_some() {
                    "instruction_write".to_string()
                } else {
                    "unknown".to_string()
                },
                seq: best_seq,
                confidence: if model_won {
                    "modeled".to_string()
                } else if best_seq.is_some() {
                    "exact".to_string()
                } else {
