...
impl TraceEngine {
    ...
    pub fn get_analysis_tainted_seqs(
        &self,
        session_id: &str,
        analysis_id: &str,
    ) -> Result<Vec<u32>> {
        let record = self.get_analysis(session_id, analysis_id)?;
        if record.kind != "backward_taint" {
            return Err(TraceError::InvalidArgument(format!(
                "Analysis {analysis_id} has kind {}, expected backward_taint",
                record.kind
            )));
        }
        record
            ...
            .ok_or_else(|| {
                TraceError::InvalidArgument(
                    "Analysis predates analysis-scoped taint pagination; rerun taint_analysis"
                        .to_string(),
                )
            })?
            .iter()
                ...
                value
                    ...
                    .ok_or_else(|| {
                        TraceError::InvalidArgument(format!(
                            "Analysis {analysis_id} contains an invalid tainted sequence"
                        ))
                    })
            })
            .collect()
...
mod tests {
    ...
    #[test]
    fn taint_sequence_pages_are_isolated_by_analysis_id_and_persisted() {
        let (engine, session_id, path) = setup();
        let first = engine
            .save_analysis(
                &session_id,
                ...
                "first taint",
                Value::Null,
                serde_json::json!({"tainted_seqs": [1, 3, 5]}),
                AnalysisEvidence::default(),
            )
            .unwrap();
        let second = engine
            .save_analysis(
                ...
                "second taint",
                Value::Null,
                serde_json::json!({"tainted_seqs": [2, 4]}),
                AnalysisEvidence::default(),
            )
            .unwrap();
        assert_eq!(
            engine
                .get_analysis_tainted_seqs(&session_id, &first.analysis_id)
                .unwrap(),
            [1, 3, 5]
        );
        assert_eq!(
            engine
                .get_analysis_tainted_seqs(&session_id, &second.analysis_id)
                .unwrap(),
            [2, 4]
        );
        engine.close_session(&session_id).unwrap();
        ...
        assert_eq!(
            reopened
                .get_analysis_tainted_seqs(&session.session_id, &first.analysis_id)
                .unwrap(),
            [1, 3, 5]
        );
        reopened.delete_file_cache(path.to_str().unwrap());
        reopened.close_session(&session.session_id).unwrap();
        let _ = std::fs::remove_file(path);
    }
