...
use crate::parser::{
    ...
    find_reg_value_u128, first_data_reg_name, is_simd_reg_name, parse_hex_u64, parse_operands_into,
    simd_arrangement_total_width,
};
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct CallHexdumpObservation {
    pub base_addr: u64,
...
impl CallAnnotation {
    pub fn hexdump_observations(&self) -> Vec<CallHexdumpObservation> {
        let mut result = Vec::new();
        let mut current: Option<CallHexdumpObservation> = None;
        for line in &self.raw_lines {
            let trimmed = line.trim();
            if let Some(rest) = trimmed.strip_prefix("hexdump at address 0x") {
                ...
                let declared_length = rest.find("length 0x").and_then(|pos| {
                    let value: String = rest[pos + 9..]
                        .chars()
                ...
                if let Some(base_addr) = base {
                    current = Some(CallHexdumpObservation {
                        base_addr,
                        declared_length,
                        bytes: Vec::new(),
            ...
            if let Some(colon) = trimmed.find(": ") {
                if trimmed[..colon].chars().all(|c| c.is_ascii_hexdigit()) {
                    let end = trimmed.find('|').unwrap_or(trimmed.len());
                    for token in trimmed[colon + 2..end].split_whitespace() {
                        if let Ok(byte) = u8::from_str_radix(token, 16) {
                            observation.bytes.push(byte);
                        }
                    }
                }
            }
    ...
    pub fn summary(&self) -> String {
        let decoded_args: Vec<String> = self
            .args
            .iter()
            .map(|(_, v)| {
...
fn parse_call_func(rest: &str, is_jni: bool) -> Option<SpecialLine> {
    ...
    Some(SpecialLine::CallFunc {
        name: name.to_string(),
        is_jni,
        raw_args: parse_call_arguments(rest),
    })
}

pub fn parse_call_arguments(text: &str) -> Vec<String> {
    text.find('(')
...
fn parse_line_gumtrace_inner(raw: &str, extract_regs: bool) -> Option<ParsedLine> {
    ...
    let insn_text = std::str::from_utf8(&bytes[insn_start..insn_end])
        .ok()?
        .trim();
...
fn find_reg_value_with_alias(bytes: &[u8], reg_name: &str, start_pos: usize) -> Option<u64> {
    find_reg_value(bytes, reg_name.as_bytes(), start_pos).or_else(|| {
...
mod tests {
    ...
    fn test_gumtrace_ldp_mem_w_with_alias() {
        ...
        assert_eq!(
            mem.value,
            Some(0x798484e190),
            "x29 loaded value via fp alias after ->"
        );
        assert_eq!(
            mem.value2,
            Some(0x7a39cae298),
            "x30 loaded value via lr alias after ->"
        );
    }
    ...
    #[test]
    fn parses_hexdump_base_length_and_bytes() {
        let ann = CallAnnotation {
            ...
            ret_value: None,
            raw_lines: vec![
                "hexdump at address 0x7fd84c7ad0 with length 0x12:".into(),
                "7fd84c7ad0: 31 32 33 34 35 36 37 38 39 30 61 62 63 64 65 66 |1234567890abcdef|"
                    .into(),
                "7fd84c7ae0: 67 68                                           |gh              |"
                    .into(),
            ],
            observation_seq: Some(100),
            completion_seq: Some(105),
        };
        let observations = ann.hexdump_observations();
        assert_eq!(observations[0].base_addr, 0x7fd84c7ad0);
        assert_eq!(observations[0].declared_length, Some(0x12));
        assert_eq!(observations[0].bytes, b"1234567890abcdefgh");
        assert_eq!(observations[0].observation_seq, Some(100));
        assert_eq!(observations[0].completion_seq, Some(105));
    }
    ...
    fn test_parse_gumtrace_no_semicolon_with_arrow() {
        let raw =
            "[libsscronet.so] 0x7a39fa11e4!0x5a01e4 mov x0, x1 x0=0xdead x1=0xbeef -> x0=0xbeef";
        let line = parse_line_gumtrace(raw).unwrap();
