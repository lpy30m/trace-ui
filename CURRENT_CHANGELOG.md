...
- 新增严格的 `trace-ui/case-v1` `.traceui-case` 工作区：保存 artifact 流式 SHA-256、大小、mtime、相对/绝对路径、父 artifact provenance 和严格 parser 摘要；支持 Trace、exact AArch64 ELF、runtime attestation、Frida、Unicorn、angr、IDA、OLLVM、coverage reconciliation、analysis、crypto KAT 与 crypto 报告。runtime attestation 必须绑定唯一 static-binary parent；coverage report 必须绑定唯一 exact ELF parent 和至少一个动态/source parent；即使验证冲突也允许导入并保存为反证。非 AArch64 ELF 会在导入时拒绝。
...
- MCP 新增 `generate_coverage_reconciliation_script` 与 `inspect_coverage_reconciliation`，连同 KAT、capture plan、ABI、benchmark、案件、Evidence Pack 与 runtime attestation 工具后当前总数 80；health capability `coverage_aware_claim_gate` 与 server instructions 已同步。Tauri 同步提供 coverage 生成/保存/检查命令。
...
- 新增 analysis-case/Replay Doctor、runtime attestation、Crypto KAT、ABI inference、Evidence Pack、capture plan、coverage claim gate 与 accuracy benchmark 定向测试；MCP 覆盖 80-tool registry/边界测试，并实际调用 coverage 脚本生成、exact ELF/source SHA 完整检查、KAT 创建/复验、ABI 推断、capture plan 与 benchmark handler。前端 Vitest 覆盖 coverage 生成/保存/检查/双 parent 导入，并确认不会调用自动 angr/目标执行接口。CI 运行 MCP lib tests，并在前端 build 后实际 `cargo build -p trace-cli -p trace-ui`。保留 ignored 真实 AES fixture 回归。Frida、Unicorn、angr、IDA 和目标程序仍保持用户手动执行边界；OLLVM/模拟结论仍为 Candidate/Related。
