...
- 新增严格的 `trace-ui/case-v1` `.traceui-case` 工作区：保存 artifact 流式 SHA-256、大小、mtime、相对/绝对路径、父 artifact provenance 和严格 parser 摘要；支持 Trace、exact AArch64 ELF、runtime attestation、Frida、Unicorn、angr、IDA、OLLVM、analysis、crypto KAT 与 crypto 报告。runtime attestation 必须绑定唯一 static-binary parent；即使验证冲突也允许导入并保存为反证。非 AArch64 ELF 会在导入时拒绝。
- GUI 在“分析历史”内新增“案件 / Replay Doctor”二级页签，可新建/打开案件、批量导入 artifact、保存生成结论、运行 AES 未识别诊断，并显示时间线、完整性、下一步、Claim 账本、模拟状态和实验矩阵。
- 案件 GUI 新增“运行时镜像认证（手动 Frida）”：选择案件中的 exact ELF parent 与本地 ELF 路径，设置 window/max-windows，生成/保存 `.js`，选择用户捕获文件严格检查覆盖和冲突，并一键导入；`refuted` 结果不会被丢弃。Tauri invoke handler 当前 100 个命令。
...
- MCP 新增 `verify_crypto_semantic_kat`、`inspect_crypto_semantic_kat`、`plan_analysis_case_capture`、`infer_frida_abi`、`run_accuracy_benchmark`，连同案件、Evidence Pack 与 runtime attestation 工具后当前总数 78；health capabilities 与 server instructions 已同步。Tauri 同步提供 KAT 生成/保存/检查、capture plan、ABI 推断与 benchmark 命令。
...
- 新增 analysis-case/Replay Doctor、runtime attestation、Crypto KAT、ABI inference、Evidence Pack、capture plan 与 accuracy benchmark 定向测试；MCP 覆盖 78-tool registry/边界测试，并实际调用 KAT 创建/复验、ABI 推断、capture plan 与 benchmark handler。CI 运行 MCP lib tests，并在前端 build 后实际 `cargo build -p trace-cli -p trace-ui`。保留 ignored 真实 AES fixture 回归和 GUI/Vitest 覆盖。Frida、Unicorn、angr、IDA 和目标程序仍保持用户手动执行边界；OLLVM/模拟结论仍为 Candidate/Related。
